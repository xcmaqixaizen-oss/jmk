    ...
    modal.innerHTML = `
      ...
      <div style="padding:24px">
        ...
        <div style="display:flex;gap:10px;flex-wrap:wrap">
          ...
          ` : ''}
          <button onclick="BookingPage.sendToWhatsApp('${b.id}')"
            style="padding:12px 20px;border-radius:var(--radius);background:#25D366;border:none;color:white;font-size:14px;font-weight:700;cursor:pointer;font-family:var(--font)">
            💬 Kirim ke WhatsApp
          </button>
...
Hubungi kami: info@wisatajomokerto.com
  ...
  // ---- INTEGRASI WHATSAPP ----
  // Nomor WA admin diambil dari pengaturan admin (admin/settings.html -> field "sWA"),
  // disimpan di localStorage key 'wj-settings'. Kalau belum diisi, pakai nomor default.
  getAdminWhatsApp() {
    const settings = JSON.parse(localStorage.getItem('wj-settings') || '{}');
    let wa = (settings.whatsapp || '6282112345678').replace(/[^0-9]/g, '');
    if (wa.startsWith('0')) wa = '62' + wa.slice(1); // 08xxx -> 628xxx
    if (!wa.startsWith('62')) wa = '62' + wa; // jaga-jaga kalau admin isi tanpa kode negara
    return wa;
  },

  buildBookingWaMessage(b) {
    const tanggalFmt = new Date(b.tanggal).toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
    const totalFmt = `Rp ${b.total.toLocaleString('id-ID')}`;
    const paymentLabels = { transfer_bank: 'Transfer Bank', gopay: 'GoPay', qris: 'QRIS', ovo: 'OVO', dana: 'DANA' };

    return [
      `haloo selamat kamu adalah orng yang saya cintai dan saya sayangi ehheheh`,
      ``,
      `Saya baru saja melakukan booking dengan detail berikut:`,
      ``,
      `🎫 Kode Booking : ${b.id}`,
      ...
      `📅 Tanggal      : ${tanggalFmt}`,
      `👥 Peserta      : ${b.pax} orang`,
      `💳 Metode Bayar : ${paymentLabels[b.payment] || b.payment}`,
      `💰 Total        : ${totalFmt}`,
      ``,
      `👤 Nama Pemesan : ${b.contactName}`,
      `📱 No. HP       : ${b.contactPhone}`,
  ...
  // Dipanggil otomatis setelah booking baru dibuat, dan juga bisa dipanggil manual dari tombol di modal detail
  sendToWhatsApp(id) {
    const b = this.allBookings.find(x => x.id === id) || (this.filtered || []).find(x => x.id === id);
    if (!b) { Toast.show('Data booking tidak ditemukan.', 'error'); return; }

    const waNumber = this.getAdminWhatsApp();
    const message = this.buildBookingWaMessage(b);
    const url = `https://wa.me/${waNumber}?text=${encodeURIComponent(message)}`;

    window.open(url, '_blank');
    Toast.show('📲 Membuka WhatsApp untuk mengirim detail booking...', 'info');
  },
  ...
  submitNew() {
    ...
    // Kirim otomatis detail booking ke WhatsApp admin.
    // Dipanggil langsung (bukan di dalam setTimeout) supaya tetap dianggap browser
    // sebagai aksi user (user gesture), jadi tidak diblokir oleh popup blocker.
    this.sendToWhatsApp(booking.id);
